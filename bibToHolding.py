...
def update_holding(holding, holding_id, full_holding_string, five_forty_one, mms_id):
    ...
    # # #
    # # # print(response.content)
    if re.search(r'<errorsExist>true</errorsExist>', str(response.content)):
...
def getLocations():
    ...
    report_outfile.write(report_str)

    report_outfile.close()

    tree = et.ElementTree(et.fromstring(report_str))
    ...
    for element in root.iter('{urn:schemas-microsoft-com:xml-analysis:rowset}Row'):
        ...
        # print ("\nElement: \n" + str(element) + "\n")
        code = element.find("{urn:schemas-microsoft-com:xml-analysis:rowset}Column2").text
        description = element.find("{urn:schemas-microsoft-com:xml-analysis:rowset}Column3").text
        library = element.find("{urn:schemas-microsoft-com:xml-analysis:rowset}Column1").text

        # print("Code: " + str(code) + "; Description: " + str(description) + "\n")
...
for mms_id in bibList:
    ...
    bib_url = 'https://api-na.hosted.exlibrisgroup.com/almaws/v1/bibs/' + str(mms_id) + '?apikey='+ secrets_local.bib_api_key


    holdings_url = 'https://api-na.hosted.exlibrisgroup.com/almaws/v1/bibs/' + str(mms_id) + '/holdings?apikey=' + secrets_local.bib_api_key
...
#    sys.exit()
    ...
    for five_forty_one in bib_record[0].get_fields('541'):
        ...
        try:
            location_541_match = re.search(r'^(.+Library|TISCH|HHSL|MUSIC|GINN|VET|Tisch|Ginn|Music|Vet|Hirsh|EUR)[ ]?(.+)?([ ]print)?[ ]+copy', subfield_3, re.IGNORECASE)
        ...
        if library_541 == "Tisch Library" or str(library_541) == "TISCH" or str(library_541) == "Tisch":
            ...
            library = "SMFA"
        elif library_541 == "Webster Family Library" or str(library_541) == "VET" or str(library_541) == "Vet":
            library = "VET"
        ...
        for full_holding in root.findall('holding'):
            ...
            holding = pym.parse_xml_to_array(io.StringIO(et.tostring(full_holding).decode('utf-8')))[0]
            ...
            elif str(location_541) == "" and holding['852']['b'] == library and library not in countList:
